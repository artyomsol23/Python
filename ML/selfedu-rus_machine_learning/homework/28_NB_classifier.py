...
a_1 = lambda x: -np.log(Dx11 * Dx12) - (x[0] - mx11) ** 2 / (2 * Dx11) - (x[1] - mx12) ** 2 / (2 * Dx12)  # -1
a1 = lambda x: -np.log(Dx21 * Dx22) - (x[0] - mx21) ** 2 / (2 * Dx21) - (x[1] - mx22) ** 2 / (2 * Dx22)   # +1
y = np.argmax([a_1(x), a1(x)]) * 2 - 1

predict = []
for x in x_train:
    predict.append(np.argmax([a_1(x), a1(x)]) * 2 - 1)
